...
public class FraudAlertEmailService {
    ...
    public void sendFraudAlertEmail(List<FraudAlert> alerts) {
        ...
        if (alerts.isEmpty()) {
            log.debug("No fraud alerts to send");
            return;
        }
        ...
        try {
            ...
            try {
                ...
                sendPlainTextEmail(alerts);
                log.info("Plain text fraud alert email sent successfully to {}", emailConfig.getRecipientEmail());
            } catch (Exception fallbackException) {
                log.error("Failed to send fraud alert email: {}", fallbackException.getMessage(), fallbackException);
            }
        }
    }
